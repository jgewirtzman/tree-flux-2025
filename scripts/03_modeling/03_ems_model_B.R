...
OUTPUT_DIR <- "outputs/models/ems_bgs_drivers"
...
# ════════════════════════════════════════════════════════════
# FORCING EXACT BGS CORE DRIVERS
# These are the exact same variables used in the BGS wetland model
# ════════════════════════════════════════════════════════════

CORE_TS_VAR <- "TS_Ha2"           # Same as BGS
CORE_WTD_VAR <- "bvs_wtd_cm"      # Same as BGS (water table, not SWC)

cat("\n─────────────────────────────────────────────────────\n")
cat("⚠ FORCING EXACT BGS CORE DRIVERS (for direct comparison):\n")
cat("  Temperature:", CORE_TS_VAR, "\n")
cat("  Water table:", CORE_WTD_VAR, "\n")
cat("─────────────────────────────────────────────────────\n")

# Check completeness of BGS drivers in upland period
cat("\nCompleteness of BGS drivers during upland flux period:\n")
for (v in c(CORE_TS_VAR, CORE_WTD_VAR)) {
  if (v %in% names(upland_period)) {
    n_total <- nrow(upland_period)
    n_valid <- sum(!is.na(upland_period[[v]]))
    pct <- round(n_valid / n_total * 100, 1)
    ...
    cat(sprintf("  %-25s NOT FOUND in data\n", v))
  }
}

# Update CORE_VARIABLES with BGS predictors
CORE_VARIABLES <- c(CORE_TS_VAR, CORE_WTD_VAR)
...
forced_core <- tibble(
  ...
  var_group = c("Soil Climate", "Hydrology"),
  window_hours = c(BGS_TS_WINDOW, BGS_WTD_WINDOW),
  r = NA_real_,  # Not from rolling analysis for EMS
  predictor = paste0(variable, "_raw_", window_hours, "h"),
  mode = "raw"
)

cat("\n⚠ FORCING EXACT BGS CORE DRIVERS:\n")
print(forced_core %>% dplyr::select(predictor, variable, window_hours))
...
# FORCE: Add BGS core predictors
for (i in seq_len(nrow(forced_core))) {
  v <- forced_core$variable[i]
  w <- forced_core$window_hours[i]
  if (!v %in% names(met)) {
    cat("  ⚠ Warning:", v, "not found in met data - will have missing values\n")
    next
  }
...
# Identify core interaction predictors dynamically based on CORE_TS_VAR and CORE_WTD_VAR
ts_pattern <- paste0("^", CORE_TS_VAR, "_raw_")
wtd_pattern <- paste0("^", CORE_WTD_VAR, "_raw_")

ts_pred <- names(model_data_scaled)[grepl(ts_pattern, names(model_data_scaled))][1]
wtd_pred <- names(model_data_scaled)[grepl(wtd_pattern, names(model_data_scaled))][1]

cat("Core interaction predictors (from BGS model):\n")
cat("  Temperature:", ts_pred, "\n")
cat("  Water table:", wtd_pred, "\n\n")

if (is.na(ts_pred) || is.na(wtd_pred)) {
  stop("Required core predictors (", CORE_TS_VAR, " and/or ", CORE_WTD_VAR, ") not found!")
}
...
if (length(high_vif_preds) > 0) {
  ...
  cat("  3. Core to your hypothesis (", CORE_TS_VAR, ",", CORE_WTD_VAR, ")\n")
  cat("─────────────────────────────────────────────────────\n")
...
for (sp in other_species) {
  ...
  wtd_sp_term <- paste0(wtd_pred, ":species", sp)
  int_sp_term <- paste0(ts_pred, ":", wtd_pred, ":species", sp)
  
  temp_sp <- temp_ref + ifelse(temp_sp_term %in% names(coefs), coefs[temp_sp_term], 0)
  wtd_sp <- wtd_ref + ifelse(wtd_sp_term %in% names(coefs), coefs[wtd_sp_term], 0)
...
coef_df <- coef_df %>%
  mutate(
    ...
    term_clean = case_when(
      ...
      term_clean == "Water table" ~ paste0("Soil moisture (", ref_species_name, ")"),
...
for (sp in species_levels) {
  for (i in seq_along(wtd_percentiles)) {
    ...
    pred_df$swc_percentile <- wtd_percentiles[i]
...
message("══════════════════════════════════════════════════")
message("")
message("Site: EMS (Upland) - USING EXACT BGS CORE DRIVERS")
message("")
message("Final model:")
...
for (i in 1:nrow(species_slopes)) {
  message(sprintf("  %s:  Temp=%.3f  WTD=%.3f  Interaction=%.3f",
                  species_slopes$species[i], 
                  species_slopes$temp_slope[i], 
                  species_slopes$wtd_slope[i], 
