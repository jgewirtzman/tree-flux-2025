...
OUTPUT_DIR <- "outputs/figures/rolling_correlations"
...
for (analysis_type in ANALYSES) {
  ...
  p_bars <- ggplot(best_for_plot, 
    ...
    labs(
      title = paste("Optimal Window Correlations with CH4 Flux", title_suffix),
...
# ============================================================
# ROLLING CORRELATION RESULTS FOR MANUSCRIPT
# ============================================================

message("\n", paste(rep("=", 60), collapse = ""))
message("ROLLING CORRELATION RESULTS FOR MANUSCRIPT")
message(paste(rep("=", 60), collapse = ""))

# Focus on anomaly-filtered results (removes seasonal confounds)
message("\n--- ANOMALY-FILTERED SIGNIFICANT CORRELATIONS ---")

sig_anom <- best_windows_all %>%
  ...
  mutate(
    ...
    window_label = case_when(
      ...
      TRUE ~ paste0(window_hours / 24, "d")
    )
  ) %>%
  dplyr::select(site, var_group, var_label, window_label, window_hours, r, p_adj)

print(sig_anom, n = 50)

# Count by site
message("\n--- Count of Significant Variables by Site ---")
sig_anom %>%
  group_by(site) %>%
  summarise(
    n_significant = n(),
...
# Count by variable group
message("\n--- Significant Variables by Group ---")
sig_anom %>%
  group_by(site, var_group) %>%
  summarise(
    n = n(),
    vars = paste(var_label, collapse = ", "),
    .groups = "drop"
  ) %>%
  print(width = 120)

# Strongest correlations by site
message("\n--- Top 5 Correlations by Site (Anomaly) ---")
sig_anom %>%
  group_by(site) %>%
  slice_head(n = 5) %>%
  print()

# Window timescales summary
message("\n--- Optimal Window Timescales ---")
sig_anom %>%
  group_by(site) %>%
  summarise(
    median_window_hours = median(window_hours),
    range_hours = paste0(min(window_hours), "-", max(window_hours)),
    .groups = "drop"
  ) %>%
  print()

# Comparison: which variables significant at both sites?
message("\n--- Variables Significant at Both Sites ---")
wetland_vars <- sig_anom %>% filter(site == "Wetland") %>% pull(var_label)
upland_vars <- sig_anom %>% filter(site == "Upland") %>% pull(var_label)
both_sites_vars <- intersect(wetland_vars, upland_vars)
message("Both sites: ", paste(both_sites_vars, collapse = ", "))
message("Wetland only: ", paste(setdiff(wetland_vars, upland_vars), collapse = ", "))
message("Upland only: ", paste(setdiff(upland_vars, wetland_vars), collapse = ", "))

# RAW results for comparison
message("\n--- RAW SIGNIFICANT CORRELATIONS (for comparison) ---")
sig_raw <- best_windows_all %>%
  ...
  dplyr::select(site, var_group, var_label, window_label, r, p_adj)
...
# Key finding: what persists after anomaly filtering?
message("\n--- Variables Significant in Both Raw AND Anomaly ---")
raw_sig_vars <- best_windows_all %>% 
  filter(analysis == "raw", significant) %>%
  dplyr::select(site, variable)
anom_sig_vars <- best_windows_all %>% 
  filter(analysis == "anomaly", significant) %>%
  dplyr::select(site, variable)

both_analyses <- inner_join(raw_sig_vars, anom_sig_vars, by = c("site", "variable"))

best_windows_all %>%
  filter(analysis == "anomaly", significant) %>%
  semi_join(both_analyses, by = c("site", "variable")) %>%
  arrange(site, desc(abs(r))) %>%
  dplyr::select(site, var_label, r, window_hours) %>%
  mutate(r = round(r, 3)) %>%
  print(n=100)
