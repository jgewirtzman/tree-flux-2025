...
pred_species <- pred_species %>%
  mutate(
    wtd_percentile = factor(wtd_percentile, levels = rev(wtd_percentiles)),
...
p_species <- ggplot(pred_species, aes(x = s10t, y = fit, color = wtd_percentile)) +
  ...
  scale_color_manual(
    values = wtd_colors[as.character(rev(wtd_percentiles))],
    labels = paste0(rev(wtd_percentiles), "%"),
    name = "Water table\npercentile"
  ) +
  labs(
...
legend_data <- data.frame(
  ...
  percentile = factor(wtd_percentiles, levels = rev(wtd_percentiles))
)

p_legend <- ggplot(legend_data, aes(x = x, y = y, fill = percentile)) +
  geom_tile() +
  scale_fill_manual(
    values = setNames(wtd_colors_loc, as.character(wtd_percentiles))[as.character(rev(wtd_percentiles))],
    labels = paste0(rev(wtd_percentiles), "%"),
    name = "Water table\npercentile"
  ) +
  theme_void() +
...
pred_wetland <- results_location[["wetland"]]$predictions %>%
  mutate(wtd_percentile = factor(wtd_percentile, levels = rev(wtd_percentiles)))
...
p_wetland <- ggplot(pred_wetland, aes(x = s10t, y = fit, color = wtd_percentile)) +
  ...
  scale_color_manual(
    values = setNames(wetland_colors, as.character(wtd_percentiles))[as.character(rev(wtd_percentiles))],
    labels = paste0(rev(wtd_percentiles), "%"),
    name = "Water table\npercentile"
  ) +
  scale_fill_manual(
