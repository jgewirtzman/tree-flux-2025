...
ggplot(ymf, aes(x=CO2_flux*1000))+
...
# Compute mean CO2_flux for each species
mean_plot <- ymf %>%
  group_by(Species.Code) %>%
  summarise(mean_flux = mean(CO2_flux, na.rm = TRUE)) %>%
  ggplot(aes(x = Species.Code, y = mean_flux, fill = Species.Code)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(y = "Mean CO2 Flux (umol CO2/m²/s)", x = "Species", title = "Mean CO2 Flux per Species") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Density plot with pseudo-log transformation
density_plot <- ggplot(ymf, aes(x = pseudo_log(CO2_flux), color = Species.Code)) +
  geom_density() +
  theme_minimal() +
  labs(x = "Pseudo-Log Transformed CO2 Flux", y = "Density", title = "Density of CO2 Flux (Transformed)") +
  theme(legend.position = "bottom")

# Histogram with colored points for values above/below zero
histogram_plot <- ggplot(ymf, aes(x = CO2_flux)) +
  geom_histogram(binwidth = 10, fill = "grey70", color = "black") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_point(data = ymf %>% filter(CO2_flux < 0), aes(y = 0, x = CO2_flux), color = "red", size = 2) +
  geom_point(data = ymf %>% filter(CO2_flux > 0), aes(y = 0, x = CO2_flux), color = "blue", size = 2) +
  facet_wrap(~Species.Code, scales = "free") +
  theme_minimal() +
  labs(x = "CO2 Flux (CO2/m²/s)", y = "Count", title = "Histogram of CO2 Flux by Species")
...
summary_stats <- ymf_cleaned %>%
  ...
  summarise(
    ...
    se_flux = sd(CO2_flux, na.rm = TRUE) / sqrt(n()),  # Standard Error
    n = n()
  ) %>%
  mutate(
...
# Create the plot
mean_plot <- ggplot(summary_stats, aes(x = Species.Label, y = mean_flux, fill = mean_flux)) +
  ...
  theme_minimal() +
  labs(y = "Mean CO2 Flux (nmol CO2/m²/s)", x = "Species", title = "Mean CO2 Flux per Species (with SE)") +
