...
# Convert flux data directly from μmol to mg C-CH4/m2/h
summary_stats_converted <- summary_stats %>%
  mutate(
    mean_flux_mg_c = convert_ch4_flux(mean_flux),
    se_flux_mg_c = convert_ch4_flux(se_flux)
  )

# Create plot with converted units
mean_plot_met <- ggplot(summary_stats_converted, aes(x = Species.Label, y = mean_flux_mg_c, 
                                                     fill = family)) +
  geom_bar(stat = "identity", color = "black") +
  geom_errorbar(aes(ymin = mean_flux_mg_c - se_flux_mg_c, ymax = mean_flux_mg_c + se_flux_mg_c),
                width = 0.2) +
  scale_fill_manual(
    name = "Plant Family",
    values = family_colors_met
